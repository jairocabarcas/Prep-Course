...
function mesesDelAño(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    if(array[i] === "Marzo"){
      ...
      if (meses.length>0 && meses[meses.length-1] === "Noviembre") {
        meses[meses.length-1] = array[i]
...
function breakStatement(numero) {
  ...
  // Tu código:
  let mensaje =[numero +2];
  for (let i = 1; i < 10; i++) {
    var suma= mensaje[i-1] + 2;
    if (suma != i){
      mensaje.push(suma);
    }else{
...
function continueStatement(numero) {
  ...
  for (let i = 1; i < 10; i++) {
    if (5 === i)continue;
    mensaje.push(mensaje[mensaje.length-1] + 2);
  }
  return mensaje
}
